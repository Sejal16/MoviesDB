...
function Favourites() {
   ...
   <div className='mt-4 px-2 flex justify-center flex-wrap space-x-2'>
      {
        genres.map((genre) =>
          <button className={
            curGenre == genre ?
              'm-2 text-lg p-1 px-2 bg-lime-600 text-white rounded-xl font-bold' :
              'm-2 text-lg p-1 px-2 bg-gray-400 hover:bg-blue-400 text-white rounded-xl font-bold'
          }
            onClick={() => {
              setCurPage(1)  /// this is imp bcoz it is not neccessary all domains have 2-3 pages so set page should be 1
              setCurGenre(genre)
            }
            }
          >
  ...
  <div className='text-center space-x-8 p-4 '>
    <input type="text"   value={search} onChange={(e) => setSearch(e.target.value)}  placeholder='Search' className='border border-4 text-center p-2 m-1 border-lime-700' />
    <input type="number" value={rows} onChange={(e) => setRows(e.target.value)} placeholder='Rows' className='border border-4 text-center  p-2 m-1 border-lime-700' />
    </div>;
...
<Pagination pageProp={curPage} forward={forward} backward={backward} />
</div>
